...
- Country Codes, while an up-to-date local JSON file as of 15th March, would have benefitted from being fetched from an API. There are a few resources online that continually keep a country codes JSON list up to date and offer it via an API call, this would mean that the list would almost always be up-to-date however I could not assume that the application would have access to the internet, therefore this specific feature was not implemented.
